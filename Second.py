...
class Items:  # definuje třídu Items a její atributy a metodu heal_user()
    ...
    def heal_user(
        self,
    ):  # pokud má hráč v inventáři věc, se kterou se chce léčit zvýší hráčovy životy o hodnotu healval a odstraní věc z hráčova inventáře, pokud hráč věc nemá, vytiskne chybovou hlášku
...
class Me:  # definuje třídu Me a její atributy a metody
    ...
    def print_inv(
        self,
    ):  # vytiskne seznam položek a jejich množství v aktuálním hráčově inventáři
    ...
    def heal(
        self,
    ):  # kontroluje, zda má hráč méně životů, než je jejich maximální počet, pokud ne, vytiskne zprávu, že se nemá cenu léčit
    ...
    def print_ava_locs(
        self,
    ):  # vypíše lokace, kam se hráč ze své aktuální lokace může dostat
    ...
    def take(self):  # zeptá se hráče kolik kamenů chce odstranit
        ...
        try:
            ...
            # pokud ne, vypíše chybovou hlášku (jaké jsou limity pro počet kamenů k odebrání) a zavolá se rekurzivně zpátky
                ...
                print(
                    repr_mess("stone_limit", "r").format(self.max_take, self.min_take)
                )
                self.take()
    ...
    def sell(self, x):  # zeptá se hráče kolik věcí (y) chce prodat
        ...
        try:
            ...
            # pokud hráč zadaný počet věcí vlastní, odečte se tento počet věcí z jeho inventáře a přičte se jejich celková prodejní hodnota k hráčově počtu peněz a o transakci se vypíše zpráva
                ...
                self.inv[x] -= y
                print(repr_mess("sell_succ", "r").format(items_list[x].sellval*y, y, x))
        # pokud hráč nezadá číslo, poprosí hráče o zadání čísla a zavolá se rekurzivně zpátky
        except ValueError:
            repr_mess("int_error", "p")
            self.sell(x)
...
def repr_mess(
    x, way
):  # načítá zprávy ze souboru mess.json v závislosti na jejím kódu (x) a způsobu načtení (way), podle x najde danou zprávu a podle way ji buď vytiskne, nebo vrátí
...
def repr_loc(
    name, key, way
):  # načítá informace o lokacích ze souboru locs.json, funguje jakožto dodatečná funkce ke třídě Location
...
def npc_check(
    npcs, npc
):  # ukládá postavy do slovníku npcs_for_loc, předměty do slovníku items_list a předměty, se kterými se hráč může léčit i do slovníku heal_list
...
me = Me(
    ...
    return_char("on_turn"),
    return_char("n"),
    return_char("min_take"),
    return_char("max_take"),
    return_char("talking"),
    return_char("last_loc"),
    return_char("inv"),
    return_char("way"),
    return_char("name"),
    return_char("safe"),
